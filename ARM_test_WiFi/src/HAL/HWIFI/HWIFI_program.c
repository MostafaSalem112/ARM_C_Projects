...
void HWIFI_voidInit(void)
{
	...
	while(Local_u8Output == 0)
	{
		MUART_vSendString((u8*)"AT+CWMODE=1\r\n");
		Local_u8Output = HWIFI_u8CommandValidate(100000,0);
	}
}
...
void HWIFI_voidConnectToNetwork (void)
{
	...
	while(Local_u8Output == 0)
	{
		MUART_vSendString((u8*)"AT+CWJAP_CUR=\"HUAWEI Y9 2019\",\"12312345\"\r\n");
		Local_u8Output = HWIFI_u8CommandValidate(100000,0);
	}
...
void HWIFI_voidConnectToServer (void)
{
	...
	while(Local_u8Output == 0)
	{
		MUART_vSendString((u8*)"AT+CIPSTART=\"TCP\",\"http://mostafasalem.freevar.com/\",80\r\n");
		Local_u8Output = HWIFI_u8CommandValidate(100000,0);
	}
...
u8 HWIFI_u8GetData (u8 *data_array)
{
	...
	MUART_vSendString((u8*)"AT+CIPSEND=46\r\n");
	status = HWIFI_u8CommandValidate(50000,0);
	
	MUART_vSendString((u8*)"GET http://mostafasalem.freevar.com/status.txt");
